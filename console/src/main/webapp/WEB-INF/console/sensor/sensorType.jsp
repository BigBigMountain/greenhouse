...
<div class="box-positon">
	<div class="rpos">当前位置: </div>
	<div style="float:right;">
		<input type="button" value="添加传感器" onclick="addEquipment()" style="line-height:25px"/>
	</div>
	<div class="clear"></div>
</div>
<div class="body-box">
		...
		<c:forEach items="${typeList}" var="type" >
			<form action="/sensor/updateType.do" method="post">
				<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
					...
					<td align="center">
						<input type="button" onclick="delEquipment(${type.id })" value="删除"/>||
	...
	<form action="/sensor/insertType.do" method="post">
		<table style="width: 100%; height:250px;font-size:18px">
			...
			<tr>
		        ...
		        <td >
		        	<select name="dataType" style="height: 30px" >
		        		<option value="0">真实值</option>
		        		<option value="1">电压值</option>
		        		<option value="2">电流值</option>
		        	</select>
		        </td>
			</tr>
			<tr>
		        <td style="text-align: right;">电信号最小值:</td>
		        <td >
		        	<input type="text" class="input-text" name="dataMin" style="height: 30px"/>
		        </td>
			</tr>
			<tr>
		        <td style="text-align: right;">电信号最大值:</td>
		        <td >
		        	<input type="text" class="input-text" name="dataMax" style="height: 30px"/>
		        </td>
			</tr>
			<tr>
		        <td style="text-align: right;">最小量程:</td>
		        <td >
		        	<input type="text" class="input-text" name="rangeMin" style="height: 30px"/>
		        </td>
			</tr>
			<tr>
		        <td style="text-align: right;">最大量程:</td>
		        <td >
		        	<input type="text" class="input-text" name="rangeMax" style="height: 30px"/>
		        </td>
			</tr>
			<tr>
		        <td style="text-align: right;">单位:</td>
		        <td >
		        	<input type="text" class="input-text" name="unit" style="height: 30px"/>
		        </td>
			</tr>
			<tr>
...
function delEquipment(id){
	...
	if(del){
		$.ajax({
			url:"/sensor/deleteType.do",
			type:"POST",
			data:{"id":id},
			success:function(data){
				//alert(data);
				window.location.href ="/sensor/typeList.do";
			}
		})
	}
}
